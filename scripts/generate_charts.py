...
def chart_ablation(results_dir: Path, chart_dir: Path) -> None:
    ...
    for bar, d in zip(bars, drops, strict=False):
        ax.text(
            bar.get_width() + 0.003,
...
def chart_eds_distribution(results_dir: Path, chart_dir: Path) -> None:
    ...
    # Reconstruct approximate distribution from real summary statistics
    # (mean/std from per_domain_breakdown.csv which is generated by the
    # real pipeline — see scripts/generate_results.py)
...
def chart_adversarial(results_dir: Path, chart_dir: Path) -> None:
    ...
    for bar, r in zip(bars, robustness, strict=False):
        ax1.text(
            bar.get_width() + 0.01,
            bar.get_y() + bar.get_height() / 2,
            f"{r:.0%}",
